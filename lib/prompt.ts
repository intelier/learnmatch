/**
 * 리포트 생성 프롬프트 v8 (D-13: 연령대 발달 단계 반영, D-14: 선택지 인용 방식 개선, D-16: 축별 이론 인용)
...
export function buildSystemPrompt(): string {
  return [
    ...
    '- ★ 신뢰도를 위해 "축별로 읽어보기"의 각 축 설명에서 관련 이론을 딱 한 문장, 자연스럽게 짚어준다. 자기주도성은 자기결정성이론(SDT)의 자율성 욕구, 유능감은 SDT의 유능감 욕구, 사회성은 SDT의 관계성 욕구, 수준 부담은 근접발달영역(ZPD), 소진 신호는 학업 소진 연구와 연결한다. 논문 인용투("~에 따르면", "~ 연구 결과")가 아니라 "이건 ~이론에서 말하는 ~예요"처럼 대화체 한 문장으로만 녹이고, 축마다 한 번을 넘지 않는다. 학습 선호 방식(스타일·깊이/넓이)은 이론적 근거가 약한 참고 지표이므로 이론을 갖다 붙이지 않는다.',
    '',
    ...
    '- 축별로 읽어보기: 자기주도성·수준 부담·소진 신호·유능감·사회성 각각 2~3문장. 각 축에서도 부모가 걱정할 만한 쪽(예: 유능감 낮음, 소진 높음, 수준 부담 높음)은 반드시 강점의 언어로 재해석해 준다. 연령대가 주어졌다면 이 시기 발달 특성과 연결해 설명한다. 위 지침대로 관련 이론을 한 문장 자연스럽게 포함한다.',
