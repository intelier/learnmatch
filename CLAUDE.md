...
**① 열람 게이트 (D-26, 지금 실제로 작동 중)**: `app/components/report-gate-screen.tsx`. 로딩이 끝나면 리포트를 바로 보여주지 않고 "리포트가 준비됐어요" 화면(목차 미리보기 + "파일럿 기간 무료로 열람하기" 버튼)을 먼저 보여준다. `result-view.tsx`의 `gatePassed` 상태로 제어 — 버튼을 눌러야 `ReportView`가 렌더된다. 나중에 결제를 붙일 땐 이 컴포넌트의 버튼만 결제 버튼으로 바꾸면 된다. `initialReport`로 전달되는 경로(공유 링크 `/r/[code]`, 예시 페이지)는 게이트를 건너뛴다.

**② 부분잠금 (D-07, 지금은 비활성 — 코드만 보존)**: `report-gate.ts`의 `splitReport()`가 `## 어쩌면 의외의 모습` 헤딩을 기준으로 마크다운을 문자열 분할한다. **프롬프트의 `## 헤딩` 문구와 이 상수가 정확히 일치해야 한다** — 헤딩을 바꾸면 게이팅이 깨진다. `PAYWALL_ENABLED=true`일 때만 켜진다(D-12 이후 미설정 = 전체 공개). 사용자가 D-26에서 "지금은 무료 기간이니 켜지 말고 준비만 해두라"고 명시적으로 결정 — 켜면 ①을 통과한 뒤에도 리포트 본문 중간부터 다시 블러 처리되는 이중 게이트가 된다(의도된 동작).
